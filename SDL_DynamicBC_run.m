function SDL_DynamicBC_run(S)


%% batch of running DynamicBC
E = F.E; % !!! F should be replaced
E = S.F.F.E; 
...
% DataProcessDir = strcat(get(E.ed_bg_data_dir,'string')); % !!! should be replaced
DataProcessDir = S.DataProcessDir;
% save_dir = get(E.ed_bg_dir_out,'string'); % !!! should be replaced
save_dir = S.save_dir;
...
%         mask_label_template = get(E.ed_mrs_files,'string'); % !!! should be replaced
        mask_label_template = S.mask_label_template;
    end
    ...
    if R1(1)
        if flag_seed_ROI(1)
%             seed_ROI_mask = get(E.rd_seed_ROI(1),'TooltipString'); % !!! should be replaced
            seed_ROI_mask = S.seed_ROI_mask;
...
set(S.F.F.pb_run_check(1),'backgroundc',get(S.F.F.hfig,'color'));
